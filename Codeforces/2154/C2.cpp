...
ll solve(const vll& a, const vll& b, ll n) {
    std::unordered_set<ll> divs;
    ll bmin1 = 1e9+9, bmin2 = 1e9+9;
    forn(i,1,n) {
        if (b[i] < bmin1) {
            bmin2 = bmin1;
            bmin1 = b[i];
    ...
    forn(i,1,n) {
        for(ll j=1; j*j<=a[i]; j++) {
            if (a[i]%j==0) {
                if (divs.find(j) != divs.end()) ans = 0;
                if (divs.find(a[i]/j) != divs.end()) ans = 0;
    ...
    forn(i,1,n) {
        ...
        for(ll k=1; k*b[i]<ans; k++) {
            for(ll j=1; j*j<=a[i]+1; j++) {
                if ((a[i]+k)%j==0) {
                    if (divs.find(j) != divs.end()) {
                        ans = min(ans, k*b[i]);
                    }
                    if (divs.find((a[i]+k)/j) != divs.end()) {
                        ans = min(ans, k*b[i]);
                    }
                }
            }
