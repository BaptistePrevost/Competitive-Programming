...
int main() {
    ...
    while(t--) {
        ...
        // Only keep non-dominating ranges (no range is contained in any other range)
        // There are at most n such ranges
        ...
        ll left = 0, right = filtered.size();
        while(right - left > 1) {
            ll mid = (left + right) / 2;
            ll leftSide = query(filtered[left].first, filtered[mid-1].second);
            ll rightSide = query(filtered[mid].first, filtered[right-1].second);
            if (leftSide > rightSide) right = mid;
            else left = mid;
        }
        ll ans = query(filtered[left].first, filtered[left].second);
        out('!') outln(ans)
    }
